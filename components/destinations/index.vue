...
<template>
  <section class="p-6 sm:p-20 sm:-mt-32 font-poppins">
    <div class="text-center space-y-3 relative">
      ...
      <h2 class="text-[#14183E] font-bold text-4xl sm:text-5xl">
        Ultime Notizie
      </h2>
      <PlusPlusSVG />
    </div>
    <div
      class="grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-12 md:gap-24 my-12  justify-center"
    >
      <div v-for="(post, index) in posts.message" :key="index" class="">
        <img
          format="png"
          :src="post.foto"
          alt="copertina-notizia"
          class="w-full h-40 object-cover rounded-t-2xl"
          width="314"
          height="457"
        />
        <div
          class="space-y-3 p-3 border-b border-r border-l rounded-x-2xl rounded-b-2xl bg-white shadow-xl"
        >
          <div class="flex items-center justify-end font-poppins">
            <!-- <p class="text-gray-500 font-medium">Rome</p> -->
            <p class="text-gray-500 font-medium">{{ post.data }} 12-02-2025</p>
          </div>
          <h2 class="text-2xl font-semibold my-3">{{ post.titolo }}</h2>
